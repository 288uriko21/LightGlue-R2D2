...
class LightGlue(nn.Module):
    default_conf = {
        ...
        "depth_confidence": 0.95,  # early stopping, disable with -1 # было 0.95 0.6
        "width_confidence": 0.99,  # point pruning, disable with -1 # было 0.99 0.55
        "filter_threshold": 0.1,  # match threshold # было 0.1
        "weights": None,
    }
    ...
    features = {
        ...
        "zippypoint": {
            "weights": "lightglue/models/weights/variables/variables",
            "input_dim": 128,
        },
        "r2d2": {
            "weights": "lightglue/models/weights/variables/variables",
    ...
    def __init__(self, features="superpoint", **conf) -> None:
        ...
        state_dict = None
        if features is not None and features != "zippypoint" and features != "r2d2":
            ...
            if features != "zippypoint" and features != "r2d2":
                path = path / "weights/{}.pth".format(self.conf.weights)
                state_dict = torch.load(str(path), map_location="cpu")
            else:
                path = "lightglue/models/weights/variables/variables.index"
                state_dict = False
